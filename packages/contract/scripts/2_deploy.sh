...
INIT_RES=$(cored tx wasm instantiate $CODE_ID "$INIT" --from my_local_dev_wallet --label "cored.in2" -b block -y --no-admin --output json  $COREUM_NODE_ARGS $COREUM_CHAIN_ID_ARGS)
