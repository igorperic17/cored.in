...
use cosmwasm_std::{from_json, to_json_binary, Binary, Coin, Deps, DepsMut, Int64, MessageInfo, QueryRequest, Response, StdError, StdResult, Uint128, Uint64};
use sha2::digest::typenum::UInt;
use crate::contract::query;
use crate::state::{did_storage_read, profile_storage, profile_storage_read, single_subscription_storage, single_subscription_storage_read, wallet_storage, wallet_storage_read, SubscriptionInfo};
...
pub fn subscribe(
    ...
    let price: Coin = from_json(get_subscription_price(deps.as_ref(), subscribe_to_did.clone())?)?;
    ...
    let subscriber_did = wallet_storage_read(deps.storage).may_load(info.sender.as_bytes())?.unwrap();
    ...
    let mut profile_info = profile_storage_read(deps.storage).may_load(subscribe_to_did.as_bytes())?.unwrap();
    profile_info.subscriber_count += Uint64::from(1u64);
    profile_storage(deps.storage).save(subscribe_to_did.as_bytes(), &profile_info).expect("Error incrementing subscriber count");
    ...
    let class_id = format!(
        "{}-{}",
        subscribe_to_did, // TODO: change to contract address
        subscribe_to_did
    );
    let nft_id = format!("{}-{}", class_id, subscriber_did.did);
...
pub fn is_subscriber(
    ...
    let subscriber_info = single_subscription_storage_read(deps.storage).may_load(key.as_bytes())?;
    ...
    // TODO: change this to {contract_address}-{profile_did}
    // issues NFTs will have IDs of the form {contract_address}-{profile_did}-{subscriber_did}
    let class_id = format!("{}-{}", did, did);
    let nft_id = format!("{}-{}", class_id, subscriber);
    let request: QueryRequest<CoreumQueries> =
        CoreumQueries::NFT(nft::Query::NFT { class_id, id: nft_id }).into();
...
pub fn set_subscription_price(
    ...
    let did_info = wallet_storage_read(deps.storage).may_load(info.sender.as_bytes()).expect("No DID registerred with the current wallet");
    println!("Set sub price: {:?}", did_info);
    let did_info_unwrapped = did_info.unwrap();
    println!("Set sub price UNWRAPPED: {:?}", did_info_unwrapped);

    let mut profile_info = profile_storage_read(deps.storage).may_load(did_info_unwrapped.did.as_bytes()).expect("Profile info for DID not found").expect("Stored profile info is empty");
    profile_info.subscription_price = Some(price.clone());
    profile_storage(deps.storage).save(did_info_unwrapped.did.as_bytes(), &profile_info).expect("Error storing the new subscription price");
